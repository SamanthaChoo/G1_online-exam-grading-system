...
def list_submissions(
    ...
    statement = (
        select(ExamAttempt)
        .where(ExamAttempt.exam_id == exam_id, ExamAttempt.submitted == True)
    ...
    for attempt in attempts:
        ...
        stmt = select(func.count(EssayQuestion.id)).where(EssayQuestion.exam_id == exam_id)
        total_questions = session.exec(stmt).one()

        # Count graded questions (where marks_awarded is not None)
        stmt = (
            select(func.count(EssaySubmission.id))
            .where(
        ...
        stmt = (
            select(func.sum(EssaySubmission.marks_awarded))
            .where(
                EssaySubmission.exam_id == exam_id,
                EssaySubmission.student_id == attempt.student_id,
            )
        ...
        stmt = select(func.sum(EssayQuestion.max_marks)).where(EssayQuestion.exam_id == exam_id)
...
def grade_student_submission(
    ...
    stmt = select(EssayQuestion).where(EssayQuestion.exam_id == exam_id).order_by(EssayQuestion.id)
...
async def save_grades(
    ...
    # Get all essay questions for this exam
    stmt = select(EssayQuestion).where(EssayQuestion.exam_id == exam_id)
    questions = session.exec(stmt).all()
